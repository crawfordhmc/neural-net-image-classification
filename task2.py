# COSC420, Helena Crawford, 14 April 2020
# Neural network for Fine-grained Classification
...
data_type = 'noisy'
(train_images, train_labels), (test_images, test_labels), class_names = load_data(task='fine-grained', dtype=data_type)
...
optn_dropout = True
# Use weight decay regularisation (True) or not (False) in the fully connected
# layers
optn_weight_decay = True
...
if os.path.isfile(model_save_name):
   ...
   if optn_data_augmentation:
      ...
      datagen = tf.keras.preprocessing.image.ImageDataGenerator(
          ...
          fill_mode='nearest',
          # no flipping as the task is to recognise standard digits
          )
...
output_test = model.predict(test_images[:16, :, :, :])
predict_test = np.argmax(output_test, axis=1)

# Show true labels and predictions for 16 test images
show_methods.show_data_images(images=test_images[:16, :, :, :],
                              labels=test_labels[:16], predictions=predict_test,
